...
def _additional_param_yes_no(self, key: str, default: str = "no") -> str:
        """
        Read yes/no-style overrides from self.additionalParameters (config-driven).
        Returns a lowercase "yes" or "no" string.
        """
        try:
            params = getattr(self, "additionalParameters", None)
        ...
        for question in questions:
            try:
                ...
                if 'driver\'s licence' in radio_text or 'driver\'s license' in radio_text:
                    ...
                    answer = _additional_param_yes_no(self, 'northKorea', default="no")

                elif 'previously employ' in radio_text or 'previous employ' in radio_text:
                    answer = _additional_param_yes_no(self, 'previouslyEmployed', default="no")
                    ...
                    answer = _additional_param_yes_no(self, 'dataRetention', default="no")
            ...
            try:
                ...
                if 'proficiency' in question_text:
                    ...
                    answer = _additional_param_yes_no(self, 'above18', default="yes")
                    choice = next((opt for opt in options if answer in opt.lower()), "")
                    if choice == "":
                        choice = options[0]
                    self.select_dropdown(dropdown_field, choice)
